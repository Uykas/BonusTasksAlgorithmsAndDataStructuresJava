...
public class bonus<T extends Comparable <T>> {
    ...
    public boolean hasTriple(T[] array, T key) { // 5
        for (int i = 0; i < array.length - 2; i++) { // time comp страшный тут емае
    ...
    private int add(T i1, T i2, T i3){
        ...
        int y = (Integer)i2;
        int h = (Integer)i3;
        for (int i = 0; i < k + y + h; i++) { // еще ішке кіргенде тагы страшно блин
            count++;
        }
        return count;
    }
